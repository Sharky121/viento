...
export default async function Page() {
    ...
    const products = data.find(({slug}: {slug: string}) => slug === "reshetki_ventilyaczionnye");
                        ...
                        {products?.subcategory.map(({title, slug}: { title: string, slug: string }, index: number) => (
                            <li key={index + uuidv4()} className="product-cards__item product-card">
                                <div className="product-card__img">
                                    <Image src={`/images/reshetki_ventilyaczionnye/${slug}/index.png`} width="180" height="180" alt="Фото продукта"/>
                                </div>
                    
                                <h3 className="product-card__title">{title}</h3>
                                <Link className="product-card__btn btn btn--outline" href={`/products/reshetki_ventilyaczionnye/${slug}`}>Подробнее</Link>
                            </li>
                        ))}
                    </ul>
                </section>
