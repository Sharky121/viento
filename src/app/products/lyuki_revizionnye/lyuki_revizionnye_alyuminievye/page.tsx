                        ...
                        {products.map(({id, title, slug}: { id: number, title: string, slug: string }) => (
                            <li key={id + uuidv4()} className="product-cards__item product-card">
                                <div className="product-card__img">
                                    <Image src={`/images/lyuki_revizionnye/lyuki_revizionnye_alyuminievye/${slug}/index.png`} width="180" height="180" alt="Фото продукта"/>
                                </div>
                    
                                <h3 className="product-card__title">{title}</h3>
                                <Link className="product-card__btn btn btn--outline" href={`/products/lyuki_revizionnye/lyuki_revizionnye_alyuminievye/${slug}`}>Подробнее</Link>
                            </li>
                        ))}
                    </ul>
                </section>
