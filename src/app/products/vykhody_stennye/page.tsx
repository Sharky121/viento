...
async function getData() {
    const response = await fetch(`${process.env.HOST}/api/wall-outlets`, {
        cache: 'no-store'
    });
        ...
        <>
            <div className="page-main__content main-content">
                ...
                <div className="main-content__header content-header">
                    ...
                    <p className="content-header__name">Выходы стенные</p>
                </div>
                        ...
                        {products.map(({title}: { title: string }, index: number) => (
                            <li key={index + uuidv4()} className="product-cards__item product-card">
                                <div className="product-card__img">
                                    <Image src={`/images/products/vykhody_stennye/${slugify(title, '_')}/index.png`} width="180" height="180" alt="Фото продукта"/>
                                </div>
                    
                                <h3 className="product-card__title">{title}</h3>
                                <Link className="product-card__btn btn btn--outline" href={`/products/vykhody_stennye/${slugify(title, '_')}`}>Подробнее</Link>
                            </li>
                        ))}
                    </ul>
                </section>
